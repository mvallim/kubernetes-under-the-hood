...
To know when a node leaves the cluster, Corosync continuously monitors the health of the node members. For this, imagine the following scenario happens:
...
* If for any reason, the node is not able to interact with a given node, then it will consider that node as unhealthy;
...
Docker is essentially a way of mapping your software in container format. But what does that mean? It means that all your software and its dependencies (like libraries, configurations, etc) are contained in a place in this case the container.

One of the great advantages here is that you can start your container in any other environment or new machines without having unexpected errors or additional configurations since everything your application needs is packaged inside the same container. In this way, the management of the container becomes: predictable, repeatable and immutable.

Can you discuss this by saying that the same could be done with virtualization? Yes, the result would be the same, but the big difference here is that we use shared OS resources where your container is running, without the need for an entire operating system hosting your application, which makes it smarter the use of the resources that application uses will use, and only its application.
...
But what happens when I have an eco-system of applications running on Docker containers? This makes our management work somewhat both unproductive and tiring, and it is a necessary scenario that the Kubernetes come in, make the management more intelligently and cleanly of our containers. (in simpler terms, Kubernetes goes beyond just managing containers, which will be dealt with in the next articles)
...
You can see more details about the Kubernetes in the previous article.
